...
function showGuideComment(sw) {
	...
	if(isEmpty(sw)) {
		if(isGuideCommentVisible) {
			...
			$guideComment.fadeIn(500,resizeGuideComment);
...
function resizeGuideComment(){
	...
	var helpWidth = $("#contents p.help > span.text > #helpSpan > input").width();
	var helpHeight = $("#contents p.help > span.text > #helpSpan > input").height();
...
function gridResize(gridId) {
	...
	gNewGridWidth = $grid.closest(".gridContainer").width()-10;	// 40 : grid 여백
...
$.openWinByName = function(obj) {
	...
	if(isNotEmpty(obj.useForm) && obj.useForm) {
		var f = eval("document."+obj.useForm);
		/*var f = (new Function("document."+obj.useForm))();*/
		if(isNotEmpty(f))
                {
                        f.action = obj.url;
                        f.target = name;
...
$(function() {
	...
	if($("#gnb").length > 0 && $(".popBody").length == 0)
	{
		...
		$("#contents").on("click","p.help", function(e) {
			showGuideComment();
			e.stopPropagation();
		});
